...
class CDATA(torch.utils.data.Dataset): # Extend PyTorch's Dataset class
    def __init__(self, opt, train, transform=None, quiet=False):
        ...
        if train:
            ...
            self.h5_img_file = np.array(h5py.File(opt['h5_img_file'], 'r')['/images_train'])
        ...
        if True:
            if self.train:
                if self.feature_type == 'VGG':
                    img = self.h5_img_file[img_idx, 0:14, 0:14, 0:512]  # [14, 14, 512]
                elif self.feature_type == 'Residual':
                    img = self.h5_img_file[img_idx, 0:14, 0:14, 0:2048] # [14, 14, 2048]
                else:
